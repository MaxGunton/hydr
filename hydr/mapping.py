...
from hydr.utils import ok_to_write, load_depfile, save_depfile
...
# TODO: Change this to get coordinates from the depfile
def export_map(depfile: str, dest: str = '.'):
    # 1) load the deployment
    deployment = load_depfile(depfile)

    hydrophones = deployment.hydrophones
    data = dict(sn=[], lat=[], lon=[], lat_s=[], lon_s=[])
    for k, v in hydrophones.items():
        data['sn'].append(k)
        data['lat'].append(v.lat)
        data['lon'].append(v.lon)
        data['lat_s'].append(v.lat_s)
        data['lon_s'].append(v.lon_s)
    ...
    # if ok_to_write(dest):
    #     kml.save(dest)
    if ok_to_write(f"{dest}/deployment_map.kml"):
        kml.save(f"{dest}/deployment_map.kml")
...
def compute_full_coord(row):
    ...
    lat = -1 * lat if row['south'] else lat
    ...
    lon = -1 * lon if row['west'] else lon
...
def set_coords(depfile):
    print("- When entering coordinates you can leave any sections blank and this will assume 0 for that component.  "
          ...
          "- If a hydrophone coordinate is missing simply leave all associated fields blank or 0.  ")
    ...
    lon_mins = []
    lon_secs = []
    for k, hydrophone in hydrophones.items():
        sn = k
        print(f"\nSerial Number: {sn}")
        # sn = 202118071
        sns.append(sn)
        print("Latitude")
        print('--------')
        if hydrophone.lat is not None:
            stored_lat = str(round(hydrophone.lat, 10)).rstrip('0')
        lad = input(f"    Degrees: {'' if hydrophone.lat is None else f'[{stored_lat}] '}")
        lad = str(hydrophone.lat) if lad == '' and hydrophone.lat is not None else lad
        # lad = '-8'
        ...
        print("Longitude")
        print('--------')
        if hydrophone.lon is not None:
            stored_lon = str(round(hydrophone.lon, 10)).rstrip('0')
        lod = input(f"    Degrees: {'' if hydrophone.lon is None else f'[{stored_lon}] '}")
        lod = str(hydrophone.lon) if lod == '' and hydrophone.lon is not None else lod
        # lod = '116'
    ...
    df = pd.DataFrame({'sn': sns, 'lat_deg': lat_degs, 'lat_min': lat_mins, 'lat_sec': lat_secs,
                       'lon_deg': lon_degs, 'lon_min': lon_mins, 'lon_sec': lon_secs, 'south': souths, 'west': wests})
    df[['lat', 'lon']] = df.apply(compute_full_coord, axis=1)

    def set_latlon(row):
        sn, lat, lon = row['sn'], row['lat'], row['lon']
        hydrophones[sn].lat = lat
        hydrophones[sn].lon = lon
    df.apply(lambda x: set_latlon(x), axis=1)
